...
async def safety_guidelines(weather_report: str, aqi_report: str) -> str:
    ...
    prompt = f"""You are a health assistant. Given this weather and air quality:
...
4. Special advice for sensitive groups."""
    ...
    try:
        ...
        if model_type == "openvino":
            ...
            def run_generation():
                return generator.generate(prompt, config).strip()
            ...
            def run_transformers():
                ...
                return output[0]["generated_text"].strip() if output else "Failed to generate safety analysis."
            
            return await loop.run_in_executor(None, run_transformers)
        else:
            return "No model available for text generation."

    except Exception as e:
        logger.error(f"Model pipeline error: {e}")
        return f"Model pipeline error: {e}"
